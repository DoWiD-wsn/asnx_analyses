#####
# @brief    Run the dDCA on a pre-recorded dataset with contextually weighted assessment
#
...
# => $ python3 simulate_ddca_context.py datasets/asnx_base_data.csv
#
# @file     simulate_ddca_context.py
...
for CSV_INPUT in csv_files:
    ...
    CSV_OUTPUT = RESULT_DIR+(Path(CSV_INPUT).stem).replace('-ddca','') + '-ddca_context.csv'
    ...
    for row in csv_i:
        if line_count>0:
            ...
            sum_w = 0
            for dc in dcs:
                sum_w += abs(dc["context"]) if dc["context"]>=0 else 1
                state = state + abs(dc["context"]) if dc["context"]>=0 else state
            state = state/sum_w
