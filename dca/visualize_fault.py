#####
# @brief    Visualize a given fault signature
#
# Python script to generate a SVG graph of the data of a given fault
# signature via matplotlib. It generates one figure with two subplots:
# 1.) effect on case-related data (air/soil temperature/humidity)
...
#
# The fault signature has to be given as parameter, for example:
# => $ python3 visualize_fault.py fault_signatures/fault-bad_connection.csv 1
# where the 2nd parameter defines the transparency of the SVG output
#
# @file     visualize_fault.py
...
# plot data
lns1 = ax1.plot(tstmp, Tair, '-',  label=r"$T_{air}$", color="darkgreen")
lns2 = ax1.plot(tstmp, Tsoil, '-',  label=r"$T_{soil}$", color="limegreen")
lns3 = ax1b.plot(tstmp, Hair, '-',  label=r"$H_{air}$", color="darkblue")
lns4 = ax1b.plot(tstmp, Hsoil, '-',  label=r"$H_{soil}$", color="dodgerblue")
...
ax2b.yaxis.set_major_locator(MultipleLocator(1))
ax2b.yaxis.set_minor_locator(AutoMinorLocator(5))
# plot data
lns1 = ax2.plot(tstmp, Xnt, '-',  label=r"$\chi_{NT}$", color="midnightblue")
lns2 = ax2.plot(tstmp, Xvs, '-',  label=r"$\chi_{VS}$", color="darkgreen")
lns3 = ax2.plot(tstmp, Xbat, '-',  label=r"$\chi_{BAT}$", color="rosybrown")
lns4 = ax2.plot(tstmp, Xart, '-',  label=r"$\chi_{ART}$", color="orangered")
lns5 = ax2.plot(tstmp, Xrst, '-',  label=r"$\chi_{RST}$", color="fuchsia")
lns6 = ax2.plot(tstmp, Xic, '-',  label=r"$\chi_{IC}$", color="lime")
lns7 = ax2.plot(tstmp, Xadc, '-',  label=r"$\chi_{ADC}$", color="aqua")
lns8 = ax2.plot(tstmp, Xusart, '-',  label=r"$\chi_{USART}$", color="gold")
lns9 = ax2b.plot(tstmp, label, '-',  label="fault label", linewidth=1, color="cornflowerblue")
