#####
# @brief    Compare the effect of the optimization concerning -- DC population size
#
...
# calculation options concerning the DC population size. The results
# are plottet via matplotlib where the focus is on the aspect to compare
# like, in this case, the DC population size.
#
...
SVG_OUTPUT = OUT_DIR+Path(CSV_INPUT).stem + "-opt_dc_pop_size-plot.svg"
...
for i in range(len(snid)):
    ...
    state = state/len(dcs_1)
    context1_t = 1 if state>0.5 else 0
    context1.append(context1_t)
    # option 2
    state = 0
    for dc in dcs_2:
        state = state + 1 if dc["context"]>=0 else state
    state = state/len(dcs_2)
    context2_t = 1 if state>0.5 else 0
    context2.append(context2_t)
    # option 3
    state = 0
    for dc in dcs_3:
        state = state + 1 if dc["context"]>=0 else state
    state = state/len(dcs_3)
    ...
    for dc in dcs_4:
        state = state + 1 if dc["context"]>=0 else state
    state = state/len(dcs_4)
    context4_t = 1 if state>0.5 else 0
...
###
lns3 = ax3.plot(index, context1, '-',  label="Cx N=1", color="thistle")
lns4 = ax3.plot(index, context2, '-',  label="Cx N=3", color="violet")
lns5 = ax3.plot(index, context3, '-',  label="Cx N=5", color="fuchsia")
lns6 = ax3b.plot(index, context4, '-',  label="Cx N=10", color="darkorchid")
###
