#####
# @brief    Compare the effect of the optimization concerning -- safe indicator difference
#
...
# calculation options concerning the safe indicator difference. The
# results are plottet via matplotlib where the focus is on the aspect to
# compare like, in this case, the safe indicator difference.
#
# The following options are compared:
# *) negative exponential
# *) inverse exponential (variant 1)
# *) inverse exponential (variant 2)
# *) inverse sigmoid
# 
# @file     opt_safe_ind_delta.py
...
SVG_OUTPUT = OUT_DIR+Path(CSV_INPUT).stem + "-opt_safe_ind_delta-plot.svg"
...
for i in range(len(snid)):
    ...
    safe4_t = safe4_dev
    # aggregate
    safe1_sig  = round(math.exp(-max(safe1_t, safe2_t, safe3_t, safe4_t)*SAFE_SENS),2)
    safe1.append(safe1_sig)
    ### Var 2 - absolute difference ###
    safe1_diff = 0
    if len(t_air_a)>1:
        safe1_diff = abs(t_air_a[-2] - t_air_a[-1])
    safe2_diff = 0
    if len(t_soil_a)>1:
        safe2_diff = abs(t_soil_a[-2] - t_soil_a[-1])
    safe3_diff = 0
    if len(h_air_a)>1:
        safe3_diff = abs(h_air_a[-2] - h_air_a[-1])
    safe4_diff = 0
    if len(h_soil_a)>1:
        safe4_diff = abs(h_soil_a[-2] - h_soil_a[-1])
    # aggregate
    safe2_sig  = round(math.exp(-max(safe1_diff, safe2_diff, safe3_diff, safe4_diff)*SAFE_SENS),2)
    safe2.append(safe2_sig)
    ### Var 3 - relative change ###
    safe1_diff = 0
    if len(t_air_a)>1:
        if (max(t_air_a[-2],t_air_a[-1]) != 0):
            safe1_diff = abs(t_air_a[-2] - t_air_a[-1]) / abs(max(t_air_a[-2],t_air_a[-1]))
    safe2_diff = 0
    if len(t_soil_a)>1:
        if (max(t_soil_a[-2],t_soil_a[-1]) != 0):
            safe2_diff = abs(t_soil_a[-2] - t_soil_a[-1]) / abs(max(t_soil_a[-2],t_soil_a[-1]))
    safe3_diff = 0
    if len(h_air_a)>1:
        if (max(h_air_a[-2],h_air_a[-1]) != 0):
            safe3_diff = abs(h_air_a[-2] - h_air_a[-1]) / abs(max(h_air_a[-2],h_air_a[-1]))
    safe4_diff = 0
    if len(h_soil_a)>1:
        if (max(h_soil_a[-2],h_soil_a[-1]) != 0):
            safe4_diff = abs(h_soil_a[-2] - h_soil_a[-1]) / abs(max(h_soil_a[-2],h_soil_a[-1]))
    # aggregate
    safe3_sig  = round(math.exp(-max(safe1_t, safe2_t, safe3_t, safe4_t)*SAFE_SENS),2)
...
###
lns2 = ax3.plot(index, safe1, '-.',  label="safe #1", color="palegreen")
lns3 = ax3.plot(index, safe2, '-.',  label="safe #2", color="lime")
lns4 = ax3.plot(index, safe3, '-.',  label="safe #3", color="forestgreen")
###
lns5 = ax3b.plot(index, context, '-',  label="fault context", color="darkorchid")
lns6 = ax3b.plot(index, label, ':',  label="fault label", color="cornflowerblue")
