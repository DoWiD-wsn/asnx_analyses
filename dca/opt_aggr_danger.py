#####
# @brief    Compare the effect of the optimization concerning -- danger indicator aggregation
#
...
SVG_OUTPUT = OUT_DIR+Path(CSV_INPUT).stem + "-opt_aggr_danger-plot.svg"
...
for i in range(len(snid)):
    ...
    danger1_sig  = round(min(danger1_t, danger2_t, danger3_t, danger4_t, danger5_t, danger6_t, danger7_t, danger8_t),2)
    danger1.append(danger1_sig)
    # Version 2 - max aggregation
    danger2_sig  = round(max(danger1_t, danger2_t, danger3_t, danger4_t, danger5_t, danger6_t, danger7_t, danger8_t),2)
    danger2.append(danger2_sig)
    # Version 3 - additive aggregation
    danger3_sig  = round((danger1_t + danger2_t + danger3_t + danger4_t + danger5_t + danger6_t + danger7_t + danger8_t),2)
    danger3.append(danger3_sig)
    # Version 4 - multiplicative aggregation
    danger4_sig  = round((((1 + danger1_t) * (1 + danger2_t) * (1 + danger3_t) * (1 + danger4_t) * (1 + danger5_t) * (1 + danger6_t) * (1 + danger7_t) * (1 + danger8_t))-1),2)
    danger4.append(danger4_sig)
...
# plot data
###
lns1 = ax3.plot(index, danger1, '--',  label="danger - min", color="lightcoral")
lns2 = ax3.plot(index, danger2, '-.',  label="danger - max", color="coral")
lns3 = ax3.plot(index, danger3, '-.',  label="danger - sum", color="red")
