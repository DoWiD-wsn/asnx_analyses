#####
# @brief    Compare the effect of the optimization concerning -- safe indicator activation
#
...
# calculation options concerning the safe indicator activation. The
# results are plottet via matplotlib where the focus is on the aspect to
# compare like, in this case, the safe indicator activation.
#
...
SVG_OUTPUT = OUT_DIR+Path(CSV_INPUT).stem + "-opt_safe_ind_act-plot.svg"
...
for i in range(len(snid)):
    ...
    safe_final = max(safe1_t, safe2_t, safe3_t, safe4_t)
    # Version 1 - min aggregation
    safe1_sig  = round(math.exp(-safe_final*SAFE_SENS),2)
    safe1.append(safe1_sig)
    # Version 2 - max aggregation
    safe2_sig  = round(2 - math.exp(safe_final*0.03),2)
    safe2.append(safe2_sig)
    # Version 3 - additive aggregation
    safe3_sig  = round((1 - (safe_final/20)**2),2)
    safe3.append(safe3_sig)
    # Version 4 - multiplicative aggregation
    safe4_sig  = round((1 / (1 + 2 * 0.5**(10 - safe_final))),2)
...
lns5 = ax3.plot(index, safe4, '-.',  label="safe #4", color="darkgreen")
###
